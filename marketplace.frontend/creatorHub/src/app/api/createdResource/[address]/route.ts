...
export const GET = async (request: any, { params }: { params: { address: string } }) => {
    try {
        ...
        const totalRevenue = user_data.data.purchases.reduce((acc: number, purchase: { userEarned: number }) => acc + purchase.userEarned, 0);
        ...
        return new Response(JSON.stringify({ totalRevenue, soldResources }), { status: 200 });
